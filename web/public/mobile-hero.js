(() => {
  ...
  style.textContent = `
    @media(max-width:720px){
      ...
      html.parable-mobile-safe .film-fx{display:none!important}
      html.parable-mobile-safe .film-aura{filter:none!important;opacity:.36!important}
      html.parable-mobile-safe .hero-film,
      html.parable-mobile-safe .film-stage{contain:layout paint style!important}
      html.parable-mobile-safe .film-vignette{opacity:.46!important}
      ...
      html.parable-mobile-safe .demo-ui-action,
      html.parable-mobile-safe .demo-intel-inspector,
      html.parable-mobile-safe .demo-world-chip,
      html.parable-mobile-safe .demo-lens-menu,
      html.parable-mobile-safe .demo-cut-toast{backdrop-filter:none!important}
      ...
      html.parable-mobile-safe .mobile-demo-pointer{position:absolute;z-index:95;width:17px;height:22px;left:0;top:0;opacity:0;pointer-events:none;transform:translate3d(-30px,-30px,0);transition:transform .48s cubic-bezier(.22,.82,.24,1),opacity .16s ease;filter:drop-shadow(0 2px 4px rgba(0,0,0,.55))}
      html.parable-mobile-safe .mobile-demo-pointer.is-visible{opacity:.92}
      html.parable-mobile-safe .mobile-demo-pointer svg{width:100%;height:100%;display:block}
      html.parable-mobile-safe .mobile-demo-pointer.is-clicking{transform:var(--cursor-pos) scale(.88)}
      ...
      html.parable-mobile-safe .mobile-demo-pointer.is-clicking .mobile-click-ring{animation:mobileClickRing .36s ease-out}
  ...
  const moveCursor = (x,y,click=false) => {
    ...
    cursor.style.setProperty('--cursor-pos',t);
    cursor.style.transform = t;
    cursor.classList.add('is-visible');
    if (click) {
      setTimeout(()=>cursor.classList.add('is-clicking'),330);
      setTimeout(()=>cursor.classList.remove('is-clicking'),700);
    }
  };
  ...
  const cursorCue = (i) => {
    ...
    const cue = cues[i];
    if (!cue) return;
    setTimeout(()=>moveCursor(cue[0],cue[1],cue[2]),420);
  };
