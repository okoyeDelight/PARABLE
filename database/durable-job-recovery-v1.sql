-- PARABLE Durable Job Recovery V1
-- Apply after job-admission-control-v1.sql.
-- Jobs that lose their worker after a crash/timeout are not abandoned.
-- A short recovery claim prevents duplicate sweepers; execution remains
-- protected by the normal PostgreSQL worker lease and idempotent job id.
...
create index if not exists durable_jobs_recovery_idx
  on public.durable_jobs(status,updated_at,recovery_expires_at)
...
declare
  ...
  v_recovery_limit integer;
  v_recovery_min_age_ms integer;
  v_recovered jsonb;
...
begin
  ...
  if p_action='health' then
    return jsonb_build_object(
      'ok',true,
      'engine','parable-postgres-hot-state-v4',
  ...
  if p_action='claim_recoverable_jobs' then
    v_project_id:=v_payload->>'scope_project_id';
    if v_project_id is null or v_project_id !~ '^[A-Za-z0-9_.:-]{1,220}$' then
      raise exception using errcode='22023',message='RUNTIME_RECOVERY_SCOPE_INVALID';
    end if;

    v_recovery_token:=v_payload->>'recovery_token';
    if v_recovery_token is null or v_recovery_token !~ '^[A-Za-z0-9_-]{16,180}$' then
      raise exception using errcode='22023',message='RUNTIME_RECOVERY_TOKEN_INVALID';
    end if;
    ...
    if not pg_try_advisory_xact_lock(hashtextextended('parable:jobs:recovery:'||v_capacity_scope,0)) then
      return jsonb_build_object(
        'claimed',false,
        'reason','recovery-busy',
        'jobs','[]'::jsonb
      );
    end if;

    v_recovery_limit:=greatest(1,least(100,coalesce((v_payload->>'limit')::integer,25)));
    v_recovery_min_age_ms:=greatest(0,least(3600000,coalesce((v_payload->>'min_age_ms')::integer,30000)));
    ...
    with candidates as (
      ...
      where status in ('queued','retrying')
        and updated_at<=clock_timestamp()-(v_recovery_min_age_ms*interval '1 millisecond')
        and (recovery_expires_at is null or recovery_expires_at<=clock_timestamp())
        and (
          (v_capacity_scope like 'preview:%' and project_id like v_capacity_scope||':%')
          or
          (v_capacity_scope='prod' and project_id like 'prod:%')
      ...
      limit v_recovery_limit
    ), claimed as (
      update public.durable_jobs d
    ...
    return jsonb_build_object(
      'claimed',true,
      'scope',v_capacity_scope,
      'jobs',coalesce(v_recovered,'[]'::jsonb)
    );
  end if;
