-- PARABLE Transactional Hot State V2
-- Apply after transactional-hot-state-v1.sql.
-- PostgreSQL owns mutable concurrency, rights and job-lease state.
-- Netlify Blobs owns immutable payloads/results/media and mirrors.
...
begin
  if not exists (
    ...
    if v_secret is null then
      raise exception using errcode='55000',message='PARABLE_RUNTIME_SECRET_MISSING';
    end if;

    perform vault.create_secret(
      v_secret,
      'parable_runtime_rpc_hmac_v1',
      'PARABLE signed runtime RPC HMAC material'
    );
  end if;
end $$;
...
create index if not exists runtime_request_nonces_used_idx
  on public.runtime_request_nonces(used_at);
alter table public.runtime_request_nonces enable row level security;
revoke all on public.runtime_request_nonces from anon,authenticated;
grant select,insert,update,delete on public.runtime_request_nonces to service_role;
drop policy if exists parable_server_only_deny on public.runtime_request_nonces;
create policy parable_server_only_deny
  on public.runtime_request_nonces
  as restrictive for all to anon,authenticated
  using(false) with check(false);

alter table public.rights_provenance
  add column if not exists rights_revision bigint not null default 1
  check(rights_revision>=1);
...
create table if not exists public.rights_events(
  ...
  action text not null check(action in ('declared','updated','revoked')),
  actor_id text not null,
  reason text,
  snapshot jsonb not null,
  created_at timestamptz not null default now()
);
create index if not exists rights_events_lookup_idx
  on public.rights_events(project_id,asset_sha256,rights_revision desc);
alter table public.rights_events enable row level security;
revoke all on public.rights_events from anon,authenticated;
grant select,insert,update,delete on public.rights_events to service_role;
drop policy if exists parable_server_only_deny on public.rights_events;
create policy parable_server_only_deny
  on public.rights_events
  as restrictive for all to anon,authenticated
  using(false) with check(false);

create table if not exists public.durable_job_events(
  id bigserial primary key,
...
create index if not exists durable_job_events_job_idx
  on public.durable_job_events(job_id,created_at desc);
create index if not exists durable_job_events_project_idx
  on public.durable_job_events(project_id,created_at desc);
alter table public.durable_job_events enable row level security;
revoke all on public.durable_job_events from anon,authenticated;
grant select,insert,update,delete on public.durable_job_events to service_role;
drop policy if exists parable_server_only_deny on public.durable_job_events;
create policy parable_server_only_deny
  on public.durable_job_events
  as restrictive for all to anon,authenticated
  using(false) with check(false);
...
language plpgsql
security definer
set search_path=pg_catalog,public,parable_private,extensions,vault
as $$
declare
  v_secret text;
  v_message text;
...
begin
  ...
  if p_action='health' then
    return jsonb_build_object(
      'ok',true,
      'engine','parable-postgres-hot-state-v2',
  ...
  if p_action='claim_job' then
    ...
    insert into public.durable_job_events(
      ...
      v_job.lease_token,jsonb_build_object('event','lease-claimed')
    );
  ...
  if p_action='transition_job' then
    ...
    if v_job.status='processing' then
      if coalesce(v_payload->>'lease_token','')='' or v_job.lease_token<>v_payload->>'lease_token' then
        raise exception using errcode='40001',message='JOB_LEASE_MISMATCH';
      end if;
    elsif v_target_status in ('retrying','succeeded') then
      raise exception using errcode='40001',message='JOB_NOT_PROCESSING';
    end if;
  ...
  if p_action='ensure_job' then
    ...
    if found then
      if v_job.project_id<>v_project_id
        ...
        raise exception using errcode='40001',message='JOB_IDEMPOTENCY_CONFLICT';
      end if;
...
drop function if exists public.parable_runtime_bootstrap(text,text);
drop table if exists parable_private.runtime_bootstrap_tokens;
drop table if exists parable_private.runtime_nonces;
drop table if exists parable_private.runtime_secrets;
