-- PARABLE Transactional Hot State V3
-- Application conflicts use P0001 rather than PostgreSQL serialization_failure
-- (40001). 40001 can be retried by infrastructure, which made an intentional
-- stale-writer/lease rejection look like a timeout. PARABLE maps the explicit
-- message codes to HTTP 409 in the signed runtime adapter, so conflicts now
-- fail fast and never masquerade as transient database failures.
