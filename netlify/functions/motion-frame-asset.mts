import { readMotionFrameAsset } from './_lib/motion-frame-assets.mts';
...
export default async(request:Request)=>{
  ...
  if(!/^[a-zA-Z0-9_-]{1,96}$/.test(projectId)||!/^[a-f0-9]{64}$/.test(id)){
    return json({error:'A valid projectId and motion-frame asset id are required.'},400);
  }
  ...
  const asset=await readMotionFrameAsset(projectId,id);
  if(!asset)return json({error:'Motion-frame asset was not found.'},404);

  return new Response(asset.bytes,{
    status:200,
    headers:{
      'content-type':String(asset.metadata.media_type||'image/jpeg'),
