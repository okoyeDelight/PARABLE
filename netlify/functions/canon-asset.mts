import { readCanonAsset } from './_lib/canon-assets.mts';
...
export default async (request: Request) => {
  ...
  if (!/^[a-zA-Z0-9_-]{1,96}$/.test(projectId) || !/^[a-f0-9]{64}$/.test(id)) {
    return json({ error:'A valid projectId and canon asset id are required.' },400);
  }
  ...
  const asset = await readCanonAsset(projectId,id);
  if (!asset) return json({ error:'Canon asset was not found.' },404);

  return new Response(asset.bytes,{
    status:200,
    headers:{
      'content-type':asset.metadata.media_type,
      'content-length':String(asset.metadata.byte_length),
