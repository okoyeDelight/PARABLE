...
function latestAttemptSnapshots(events: Record<string, any>[]) {
  ...
  for (const event of events) {
    ...
    byAttempt.set(attemptId, {
      ...
      scene_id: String(event?.scene_id || event?.metadata?.scene_id || '').trim(),
      shot_id: String(event?.shot_id || event?.metadata?.shot_id || '').trim(),
...
export function evaluateRenderBudget(args: {
  ...
  if (projected.shot_attempts > policy.max_attempts_per_shot) {
    return decision(false, 'RENDER_ATTEMPT_LIMIT_REACHED',
  ...
  if (anyMoneyPolicy && args.estimatedCostUsd === null) {
    return decision(false, 'RENDER_COST_ESTIMATE_REQUIRED',
  ...
  if (
    ...
    return decision(false, 'RENDER_SHOT_BUDGET_EXCEEDED',
  ...
  if (
    ...
    return decision(false, 'RENDER_PROJECT_BUDGET_EXCEEDED',
  ...
  if (
    ...
    return decision(false, 'RENDER_COST_APPROVAL_REQUIRED',
      'This single render exceeds the human-approval threshold.', true);
  }
